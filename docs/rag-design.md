...
- **入口 A（注入）**：用户无感，`min-score=0.40` 只在此处生效——搜不到相关内容时宁可干净地不注入，也不让噪声污染 prompt。（0.25→0.40 由评测系统校准得出：无关题 Top1 全部落在 0.25~0.35，旧门槛形同虚设，误注入率 100%，见 §四 Phase1-#5）
...
### 首份基线（2026-08-30，标注集 32 相关 + 8 无关）
...
| MRR | 0.839 | ≥ 0.60 | ✓ |
| 分数区分度 | 0.249 | ≥ 0.10 | ✓ |
| 误注入率 | 0/8（min-score 0.25 时为 8/8 ❌ → 校准至 0.40）| ≤ 0.25 | ✓ |
| 答案要素命中（生成层 14 条） | 14/14 | ≥ 0.80 | ✓ |
| 引用指向正确 | 13/14 = 0.93 | ≥ 0.80 | ✓ |

> 评测的第一个战果：量化出 min-score=0.25 门槛形同虚设（无关题 Top1 落在 0.25~0.35 区间，全部被注入 prompt），校准到 0.40 后误注入清零且召回无损。唯一引用失败 case 是模型编造了超出注入数量的编号 [2]——后续可在 prompt 中强调"只允许引用给出的编号"。
