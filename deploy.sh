...
case "$ACTION" in
  ...
  backup)
    ...
    $COMPOSE run --rm --no-deps --entrypoint tar backend -C /app -cf - log | gzip > "$DEST/backend-logs.tar.gz"
    echo "  - backend 日志 → $DEST/backend-logs.tar.gz"
    $COMPOSE exec -T mysql sh -c 'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" --databases jarvis' | gzip > "$DEST/mysql-jarvis.sql.gz"
    echo "  - MySQL 数据（jarvis 库）→ $DEST/mysql-jarvis.sql.gz"
